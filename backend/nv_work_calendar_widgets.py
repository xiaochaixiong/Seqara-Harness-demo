...
class _WcDragTitleBar(QFrame):
    """无边框弹窗标题栏：左标题，右侧三色窗口控制点；支持拖动（与主程序 QFrame#topbar 一致）。"""

    def __init__(self, dialog: QDialog, title: str, palette: dict[str, str]) -> None:
        super().__init__()
        self.setObjectName("topbar")
        self._dlg = dialog
        self._drag_pos: Optional[QPoint] = None
        text_c = palette.get("text", "#111111")
        lay = QHBoxLayout(self)
        lay.setContentsMargins(12, 8, 8, 8)
        lay.setSpacing(8)
        ttl = QLabel(title)
        ttl.setStyleSheet(f"color: {text_c}; font-weight: 600; font-size: 11pt;")
        self._dots_wrap = QWidget()
        dots_l = QHBoxLayout(self._dots_wrap)
        dots_l.setContentsMargins(0, 0, 0, 0)
        dots_l.setSpacing(6)
        dots_l.setAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)

        def mk_dot(object_name: str, tooltip: str) -> QPushButton:
            b = QPushButton()
            ...
            b.setStyleSheet(
                ...
                QPushButton:hover {{
                    ...
                    border: 1px solid rgba(160,160,160,0.66);
                }}
                QPushButton#traffic_minimize:hover {{
                    background: #facc15;
                    border: 1px solid #facc15;
                }}
                QPushButton#traffic_zoom:hover {{
                    background: #22c55e;
                    border: 1px solid #22c55e;
                }}
                QPushButton#traffic_close:hover {{
                    background: #ef4444;
        ...
        # 同主界面：默认灰点，悬停按功能着色
        self._btn_min = mk_dot("traffic_minimize", "最小化")
        self._btn_max = mk_dot("traffic_zoom", "最大化/还原")
        self._btn_close = mk_dot("traffic_close", "关闭")
        self._btn_min.clicked.connect(self._dlg.showMinimized)
        self._btn_max.clicked.connect(self._toggle_max_restore)
        self._btn_close.clicked.connect(self._dlg.reject)
        dots_l.addWidget(self._btn_min)
        dots_l.addWidget(self._btn_max)
        dots_l.addWidget(self._btn_close)

        lay.addWidget(ttl, 1)
        lay.addWidget(self._dots_wrap, 0, Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
    ...
    def mousePressEvent(self, event) -> None:
        if event.button() == Qt.MouseButton.LeftButton and not self._dots_wrap.geometry().contains(
            event.pos()
        ):
            self._drag_pos = event.globalPosition().toPoint() - self._dlg.frameGeometry().topLeft()
...
class WcCalendarDialog(QDialog):
    ...
    def __init__(
        ...
        *,
        ...
        bar = _WcDragTitleBar(self, title, p)
        outer.addWidget(bar)
